...
async function readSSEAndAssembleChatCompletion(upstreamResp, reqId) {
  ...
  for await (const chunk of upstreamResp.body) {
    ...
    while ((idx = buffer.indexOf("\n")) !== -1) {
      ...
      if (delta?.content) {
        log("info", "sse_delta_content", {
          reqId,
          delta_length: delta.content.length,
          total_length: content.length + delta.content.length
        });
      ...
      if (delta?.text) {
        log("info", "sse_delta_text", {
          reqId,
          delta_length: delta.text.length,
          total_length: content.length + delta.text.length
        });
        content += delta.text;
      }
      ...
      if (delta?.reasoning_content) {
        log("info", "sse_delta_reasoning", {
          reqId,
          delta_length: delta.reasoning_content.length,
          total_length: reasoning_content.length + delta.reasoning_content.length
        });
